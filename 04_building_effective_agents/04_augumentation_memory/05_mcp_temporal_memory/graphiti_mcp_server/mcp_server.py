...
class GraphitiLLMConfig(BaseModel):
    ...
    def create_client(self) -> LLMClient:
        ...
        if not self.api_key:
            raise ValueError('GOOGLE_API_KEY must be set when using Google API')
...
async def initialize_graphiti():
    ...
    try:
        ...
        if not llm_client and config.use_custom_entities:
            ...
            raise ValueError('GOOGLE_API_KEY must be set when custom entities are enabled')
...
"""The main entry point for the Graphiti MCP server.""" 

async def run_mcp_server():
    """Initialize the server when the FastAPI app starts."""
    global config
