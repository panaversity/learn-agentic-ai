...
**Objective:** Learn how to use the `$/ping` and `$/pong` messages to check connection health and measure latency.

This is a simple but important utility for monitoring the health and responsiveness of a stateful MCP session.

## Key MCP Concepts

-   **`$/ping` (Request):** A request sent from the client to the server to check for liveness.
-   **`$/pong` (Response):** The server's immediate response to a `$/ping`. The `params` of the `$/ping` are echoed in the `result` of the `$/pong`, allowing the client to match them up.
-   **Health Check:** A client can periodically send pings to ensure the server is still responsive.
-   **Latency Measurement:** By measuring the time between sending `$/ping` and receiving `$/pong`, the client can calculate the round-trip latency of the connection.
...
-   **`server.py`:**
    -   `FastMCP` handles `$/ping` requests automatically by replying with `$/pong`, so no special server-side code is needed for this lesson. We will simply run a basic `FastMCP` server.

-   **`client.py`:**
    -   The client will establish a stateful connection.
    -   It will then send a `$/ping` request to the server.
    -   It will wait for the `$/pong` response and verify that the payload matches what it sent.
    -   It will calculate and print the round-trip time, demonstrating a simple latency check.
