...
def always_fail_activity(ctx: WorkflowActivityContext, data: str):
    logger.error(
        f"Always Fail Activity (WF '{ctx.workflow_id}'): Raising exception for input '{data}'")
...
def compensation_activity(ctx: WorkflowActivityContext, error_info: dict):
    logger.info(
        f"Compensation Activity (WF '{ctx.workflow_id}'): Compensating for error: {error_info.get('error_message')}")
...
def error_handling_workflow(ctx: DaprWorkflowContext, task_data: str):
    logger.info(
        f"ErrorHandling WF '{ctx.instance_id}': Starting with '{task_data}'")
...
async def start_wf_endpoint(user_name: str):
    ...
    try:
        instance_id = client.schedule_new_workflow(
            workflow=error_handling_workflow, input=user_name)
